...
- [ ] **HOOK-01**: Standalone `play.js` reads Claude Code hook stdin JSON and identifies the event type
- [ ] **HOOK-02**: Dispatcher maps hook events to sound mappings from `claude-peon.json`
- [ ] **HOOK-03**: Dispatcher spawns afplay asynchronously (never blocks Claude Code)
- [ ] **HOOK-04**: Dispatcher supports Stop event (task complete sound)
- [ ] **HOOK-05**: Dispatcher supports Notification event with permission/idle matchers
- [ ] **HOOK-06**: Dispatcher supports SessionStart event (startup sound)
- [ ] **HOOK-07**: Dispatcher supports UserPromptSubmit event (acknowledge sound)
- [ ] **HOOK-08**: Dispatcher supports PreToolUse event (tool-before sound)
- [ ] **HOOK-09**: Dispatcher supports PostToolUse event (tool-after sound)
- [ ] **HOOK-10**: Dispatcher consumes stdin fully and exits 0 with no stdout output
